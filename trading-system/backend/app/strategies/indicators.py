...
from app.strategies.common_indicators import (
    calculate_ma as common_ma,
    calculate_ema as common_ema,
    calculate_rsi_simple,
    calculate_macd as common_macd,
    calculate_bollinger as common_bollinger,
    calculate_volatility as common_volatility,
    MACDResult,
    BollingerBands,
    RSIResult
)
...
def calculate_ma(prices: List[float], period: int) -> float:
    return common_ma(prices, period)


def calculate_ema(prices: List[float], period: int) -> float:
    return common_ema(prices, period)


def calculate_rsi(prices: List[float], period: int = 14) -> float:
    return calculate_rsi_simple(prices, period)


def calculate_macd(prices: List[float], fast: int = 12, slow: int = 26, signal: int = 9) -> MACDResult:
    return common_macd(prices, fast, slow, signal)


def calculate_bollinger(prices: List[float], period: int = 20) -> BollingerBands:
    return common_bollinger(prices, period)


def calculate_volatility(prices: List[float]) -> float:
    return common_volatility(prices)
...
async def analyze_trend(candles: List[List], current_price: Optional[float] = None) -> TrendAnalysis:
    ...
    if 2 < recent_change < 8:
        ...
        bearish_score += 1
        signals.append(f"暴跌{recent_change:.1f}%(观望)")
